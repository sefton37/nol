...
Phase 6 and beyond are out of scope for this document but the direction is:

6. **Corpus expansion** — Write 200+ programs covering sorting, data structures, string ops (once CHAR operations are added)
7. **LLM integration** — Fine-tune or LoRA-adapt a small model on (intent → assembly) pairs
8. **Feedback loop** — LLM generates, verifier checks, failures become training signal
9. **Optimization** — If performance matters, add a simple JIT or compile to native

But none of that matters until Phases 1-5 are solid. Build the foundation right.
